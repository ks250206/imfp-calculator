...
- TUIのGraphペインは `plotters-bitmap` で画像生成し、`ratatui-image` で表示する。理由は `docs/adr/0006-image-based-tui-graph-rendering.md` を参照する。旧来の直接buffer描画や `plotters-ratatui-backend` 相当の描画経路は持たない。
- Graphペインだけ端末テーマに依存しない白背景にする。`plotters-bitmap` による画像生成は別スレッドで行い、完了までは白背景を表示する。
...
- TUIのペイン移動は `Tab` / `Shift-Tab` と数字キーで行う。Help/Logは5番ペインとしてフォーカス対象にする。`h` / `l` と左右矢印は入力フィールド内カーソル移動、preset / enum候補切り替え、Graph上のElectron energyマーカー移動に使い、ペイン移動には使わない。
- 全フォーカス可能ペインは `v` でVisual modeに入れる。
